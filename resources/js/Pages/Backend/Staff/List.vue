...
const viewDetail = item => {
    router.get(route('staff.view', item.id));
}

// Deleting selected contacts
const deleteRecords = items => {
    router.delete(route('staff.delete'), {
        data: {
            ids: items
        },
        onFinish: (d)=>{
            ...
            loadRecords(obj);
            showSnackbar("Selected staff have been deleted");
            selected.value = [];
        }
    })
}
...
const deleteDialog = {
    ...
    body: "Are you sure you want to delete the selected staff?"
}
