...
const editItem = item => {
    router.get(route('state.edit', item.id))
}

const viewDetail = item => {
    router.get(route('state.view', item.id));
}

// Deleting selected contacts
const deleteRecords = items => {
    router.delete(route('states.delete'), {
        data: {
            ids: items
        },
        onFinish: (d)=>{
            ...
            loadRecords(obj);
            showSnackbar("Selected states have been deleted");
            selected.value = [];
        }
    })
}
...
const deleteDialog = {
    ...
    body: "Are you sure you want to delete the selected states?"
}
