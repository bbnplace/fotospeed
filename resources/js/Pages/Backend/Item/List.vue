...
const editItem = item => {
    router.get(route('item.edit', item.id))
}

const viewDetail = item => {
    router.get(route('item.view', item.id));
}
...
const deleteRecords = items => {
    router.delete(route('items.delete'), {
        ...
        onFinish: (d)=>{
            ...
            loadRecords(obj);
            showSnackbar("Selected items have been deleted");
            selected.value = [];
        }
    })
}
...
const deleteDialog = {
    ...
    body: "Are you sure you want to delete the selected items?"
}
